...
public class CoarseToFineMaxRuleParser extends ConstrainedArrayParser {
	...
	public void printOutsideScores() {
		for (int i = 0; i < oScore.length; i++) {
			int row_index = 0;

			System.out.println("Token: " + i);
			for (double[] row: oScore[i][i+1]) {
				System.out.println(row_index + "\t" + Arrays.toString(row));
				row_index++;
			}
			System.out.println();
		}
	...
	public void printMaskedTokenProbabilities() {
		for (int i = 0; i < oScore.length; i++) {
			System.out.println("Token: " + i);
			...
			double token_probability = 0.;
			for (int j = 0; j < iScore[i][i+1].length; j++) {
				if (iScore[i][i+1][j] == null || oScore[i][i+1][j] == null) {
					continue;
				}
				for (int k = 0; k < iScore[i][i+1][j].length; k++) {
					token_probability += iScore[i][i+1][j][k] * (oScore[i][i+1][j][k] / normalizer);
				}
			}
			System.out.println(token_probability);
		}
	}
	...
	public double computeTightThresholds(List<String> sentence) {
		...
		for (int level = startLevel; level < endLevel; level++) {
			...
			final boolean viterbi = true, logScores = true;  // viterbi = true originally!
			if (level < 1) {
				doConstrainedViterbiInsideScores(curGrammar,
						level == startLevel);
				score = viScore[0][length][0];
