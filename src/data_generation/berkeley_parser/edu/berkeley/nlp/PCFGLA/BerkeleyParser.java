...
public class BerkeleyParser {
	...
	public static class Options {
		...
		@Option(name = "-allRootNodes", usage = "Use all possible root productions, else only ROOT -> S")
		public boolean allRootNodes;

		@Option(name = "-noOutput", usage = "Do not print the trees to stdout")
		public boolean noOutput;

		@Option(name = "-sub_pcfg", usage = "PCFG to filter current rules, overwrites original scores")
		public String sub_pcfg = null;
	}

	public static void filterGrammarRules(String file_name, Grammar grammar) {
		for (int state = 1; state < grammar.numStates; state++) {
			for (BinaryRule rule : grammar.splitRulesWithP(state)) {
				for (double[][] rows : rule.scores){
					for (double[] row: rows) {
			...
			for (UnaryRule rule : grammar.closedViterbiRulesWithParent[state]) {
				for (double[] row : rule.scores){
					if (row == null) { continue; }
					Arrays.fill(row, 0.0);
				}
			}
		...
		try {
			...
			while (scanner.hasNextLine()) {
				...
				if (split_line.length == 5) {
					// Binary Rules
					String lhs_str = split_line[0];
					String lc_str = split_line[2];
					String rc_str = split_line[3];

					State lhs = parseState(lhs_str, grammar);
					State lc = parseState(lc_str, grammar);
					State rc = parseState(rc_str, grammar);
					double prob = parseProdProb(split_line[4]);

					BinaryRule rule = grammar.getBinaryRule(lhs.tag, lc.tag, rc.tag);
					rule.scores[lc.substate][rc.substate][lhs.substate] = prob;
				} else if (split_line.length == 4) {
					// Unary Rules
					String lhs_str = split_line[0];
					String c_str = split_line[2];

					if (c_str.startsWith("'")) { continue; }

					State lhs = parseState(lhs_str, grammar);
					State c = parseState(c_str, grammar);
					double prob = parseProdProb(split_line[3]);

					// But what about UnaryRules outside the Viterbi rules? D:
					for (UnaryRule rule : grammar.closedViterbiRulesWithParent[lhs.tag]) {
						if (rule.childState == c.tag) {
							rule.scores[c.substate][lhs.substate] = prob;
							break;
						}
					}
				} else {
	...
	private static class State {
		...
		public State(short tag, short substate) {
			this.tag = tag;
			this.substate = substate;
		}
	}

	public static State parseState(String lhs, Grammar grammar) {
		String lhs_state = lhs.split("_")[0].replace("AT", "@").replace("TICK", "``").replace("DOT", ".").replace("COMMA", ",").replace("HASH", "#").replace("LRB", "-LRB-").replace("RRB", "-RRB-").replace("APOSTROPHE", "''").replace("DOLLAR", "$").replace("COLON", ":").replace("COMMA", ",").replace("PRTADVP", "PRT|ADVP");

		int lhs_tag = parseTag(lhs_state, grammar);
		int lhs_substate = Integer.parseInt(lhs.split("_")[1]);

		return new State((short) lhs_tag, (short) lhs_substate);
	}

	public static double parseProdProb(String str_prob) {
		return Double.parseDouble(str_prob.substring(1, str_prob.length() - 1));
	}

	public static int parseTag(String lhs_state, Grammar grammar) {
		try {
			return grammar.tagNumberer.number(lhs_state);
		} catch (NoSuchElementException e) {
			return grammar.tagNumberer.number(lhs_state + "^g");
		}
	}
	...
	public static void main(String[] args) {
		...
		if (opts.nGrammars != 1) {
			...
			if (opts.sub_pcfg != null) {
				filterGrammarRules(opts.sub_pcfg, grammar);
			}

			if (!opts.allRootNodes) {
				for (UnaryRule item : grammar.unaryRulesWithParent[0]) {
					if (item.childState != 1) {
						for (double[] row : item.scores) {
							Arrays.fill(row, (double)0.0);
						}
					}
				}
			}
