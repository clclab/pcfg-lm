...
def read_grammar_rules(path, threshold=1e-7):
    ...
    non_rec_rules = [
        l for l in lines if not(
            l.split()[0] in l.split()[1:] and len(l.split()) == 4
        ) 
        and not "ROOT" in l
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t','--threshold', help='Path write nltk grammar to', default=1e-7, type=float)
    ...
    print(f'{len(grammar_rules)}+{len(lexicon_rules)}={len(formatted_rules)} NLTK rules')
